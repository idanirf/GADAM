...
public class ReceptionViewController {
    ...
    private void findByName() throws SQLException {
        ...
        if(name.isEmpty()){
            ...
            tableRecept.setItems(repo.findAll().filtered(x -> x.getSupplierName().get().toLowerCase().contains(name)|| x.getSupplierName().get().toUpperCase().contains(name) || x.getRIC().contains(name)));
        }
        tableRecept.refresh();
    }
    ...
    private void onCreateAction() throws IOException {
        ...
        boolean aceptarClicked = SceneManager.initReception(recepcion,lineReception);
        if (aceptarClicked) {
            try {
                repo.save(recepcion);
                loadProd();
    ...
    private void onClickAction() throws IOException, SQLException {
        ...
        Reception x = tableRecept.getSelectionModel().getSelectedItem();
        SceneManager.initResumeReception(x);
